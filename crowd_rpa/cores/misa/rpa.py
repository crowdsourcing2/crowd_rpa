...
class MisaRpa(IRpa, ABC):
    ...
    def get_portal(self):
        url = ""
        path = misa_constant.FILE_PATH
        if path.endswith(".pdf"):
            url = util_rpa.read_pdf(path, misa_constant.URL_KEYWORD,
                                    misa_constant.LAST_KEYWORD)
        elif path.endswith(".xml"):
            url = util_rpa.read_xml(path, misa_constant.URL_KEYWORD)
        return url
    ...
    def get_code_lookup(self):
        ...
        path = misa_constant.FILE_PATH
        if path.endswith(".pdf"):
            code_bill = util_rpa.read_pdf(path, misa_constant.CODE_BILL_KEYWORD,
                                          misa_constant.URL_KEYWORD)
        elif path.endswith(".xml"):
            code_bill = util_rpa.read_xml(path, misa_constant.CODE_BILL_KEYWORD)
        return code_bill
    ...
    def process_download_xml_pdf(self):
        ...
        # Open a website
        browser.get(self.get_portal() + '?sc=' + self.get_code_lookup())
        logging.info(f'{self.get_name()}: Open a website: {self.get_portal() + "?sc=" + self.get_code_lookup()}')
